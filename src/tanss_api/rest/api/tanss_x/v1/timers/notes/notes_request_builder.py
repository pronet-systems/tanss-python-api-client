...
class NotesRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.x/v1/timers/notes
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.x/v1/timers/notes", path_parameters)
    
    def by_id(self,id: int) -> NotesItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.tanssX.v1.timers.notes.item collection
        param id: Timer-ID.
    ...
    async def put(self,body: TnsTimerFragment, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[NotesPutResponse]:
        """
        Ändert die Notiz eines Timer-Fragments (Zeitabschnitt), identifiziert über timerId/startTime/stopTime und Hash. Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Hinweise: Auch unter /api/tanss.app/v1. hash muss dem aktuellen Fragment-Hash entsprechen, sonst TnsSaveException TIMER_NOTE_HAS_BEEN_CHANGED. Wird kein Fragment gefunden, folgt ein 500. Kein Rechte-Check. meta.status UPDATED.
