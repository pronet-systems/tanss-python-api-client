...
class RecurrenceRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.x/v1/recurrence
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.x/v1/recurrence", path_parameters)
    ...
    async def post(self,body: RecurrencePostRequestBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[RecurrencePostResponse]:
        """
        Erstellt eine Wiederholungsregel (Alias zu POST /api/v1/recurrence).Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft.Token: module, Rollen TANSS_APP.Rechte: interner Mitarbeiter-Login; Zugriff auf Assignment linkTypeId/linkId (geprüft).Hinweis: Auch unter /api/tanss.app/v1. 400 RECURRENCE_RULE_MUST_HAVE_START_DATE wenn startDate fehlt/0. Berechnete Termine werden im Service erzeugt. Status CREATED.
