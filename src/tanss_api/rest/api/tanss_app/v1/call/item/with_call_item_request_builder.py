...
class WithCallItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.app/v1/call/{callId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.app/v1/call/{callId}", path_parameters)
    ...
    async def put(self,body: WithCallPutRequestBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[WithCallPutResponse]:
        """
        Aktualisiert einen bestehenden Telefonanruf (numerische TANSS-id) per JSON-Merge. Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Rechte: Lizenz adl.TELEPHONE; bet.PHONE_LIST_COMPANY oder bet.PHONE_LIST_ALL_CALLS. Hinweise: Status UPDATED. {callId} ist die numerische id, nicht die String-callId. Bei Angabe von phoneParticipants werden alle Teilnehmer ersetzt. Key "id" im Body wird ignoriert. 404 ENTITY_NOT_FOUND. Auch unter /api/tanss.x/v1.
