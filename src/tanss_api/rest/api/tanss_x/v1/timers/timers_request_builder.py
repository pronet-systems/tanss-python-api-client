...
class TimersRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.x/v1/timers
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.x/v1/timers", path_parameters)
    
    def by_id(self,id: int) -> TimersItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.tanssX.v1.timers.item collection
    ...
    async def get(self,request_configuration: Optional[RequestConfiguration[TimersRequestBuilderGetQueryParameters]] = None) -> Optional[TimersGetResponse]:
        """
        Liefert alle Timer eines Mitarbeiters (ohne Fragmente). Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Hinweise: Auch unter /api/tanss.app/v1. Query employeeId ist Pflicht (400 wenn fehlt). Kein Rechte-Check, Timer beliebiger Mitarbeiter abrufbar. unfolded stammt aus den Benutzerpräferenzen.
    ...
    async def post(self,body: TnsTimer, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[TimersPostResponse]:
        """
        Legt einen neuen Timer an und startet ihn sofort (erstes Fragment mit Notiz). Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Hinweise: Auch unter /api/tanss.app/v1. 400 TnsInvalidParameterException(employeeId) wenn employeeId=0. note ist die Notiz des ersten Fragments (Fallback title). Kein Rechte-Check, Timer kann für fremde Mitarbeiter angelegt werden. Erzeugt Log-Eintrag. meta.status CREATED.
    ...
    def to_get_request_information(self,request_configuration: Optional[RequestConfiguration[TimersRequestBuilderGetQueryParameters]] = None) -> RequestInformation:
        ...
        """
        request_info = RequestInformation(Method.GET, '{+baseurl}/api/tanss.x/v1/timers?employeeId={employeeId}', self.path_parameters)
