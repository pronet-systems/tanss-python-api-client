...
class CallRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.x/v1/call
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.x/v1/call", path_parameters)
    
    def by_call_id(self,call_id: int) -> WithCallItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.tanssX.v1.call.item collection
        param call_id: Numerische Datensatz-ID des Anrufs
    ...
    async def post(self,body: CallPostRequestBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[CallPostResponse]:
        """
        Legt einen Telefonanruf (TnsPhoneCall) für das Telefonsystem TEAMS an; das JSON wird in ein vorbelegtes Objekt gemerged und gespeichert.Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft.Token: module, Rollen TANSS_APP.Rechte: Lizenz TELEPHONE; PHONE_LIST_COMPANY; PHONE_LIST_ALL_CALLS.Hinweis: Auch unter /api/tanss.app/v1. Server setzt vorab group="TEAMS", telephoneSystemId=TELEPHONE_SYSTEM_TEAMS, checkForDuplicateCallId=true (JSON kann diese überschreiben). Gleiche callId im selben Telefonsystem -> TnsDuplicateCallIdException. Bei numberIdentifyState=UNIDENTIFIED wird Rufnummern-Identifikation ausgeführt; bei IDENTIFIED Firmenzugriff auf from/toCompanyId geprüft (403). Rufnummern werden auf 50 Zeichen gekürzt. phoneParticipants werden mit phoneCallId gespeichert. Status CREATED.
