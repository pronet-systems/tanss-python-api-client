...
class TicketRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.x/v1/ticket
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.x/v1/ticket", path_parameters)
    
    def by_ticket_id(self,ticket_id: int) -> WithTicketItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.tanssX.v1.ticket.item collection
    ...
    async def post(self,body: TicketSave, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[TicketPostResponse]:
        """
        Erstellt ein Ticket (wie POST /api/v1/tickets), jedoch ohne Auftraggeber-Prüfung (remitterCheck=false). Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Rechte: Lizenz TICKET. Hinweise: Auch unter /api/tanss.app/v1. Weitere Rechte-/Pflichtfeldprüfungen im Ticket-Service. meta.status CREATED.
    ...
    async def put(self,body: TicketConfiguration, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[TicketPutResponse]:
        """
        Liefert eine Ticketliste anhand einer TnsTicketConfiguration (identisch zu PUT /api/v1/tickets). Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Rechte: TICKET_SYSTEM, Lizenz TICKET. Hinweise: Auch unter /api/tanss.app/v1. checkPermissionsOfUser wird serverseitig auf true gesetzt; withMetaInformations/withAllListInfos/withProjectInfos laden Zusatzdaten. Die Konfiguration kennt weitere Felder als das Schema TicketConfiguration (z. B. staffOrRolePermission, includeBranches, searchText, order, fetchSummaries).
