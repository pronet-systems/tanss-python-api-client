...
@dataclass
class TnsRecurrenceAssignment(AdditionalDataHolder, Parsable):
    """
    Zuordnung einer Wiederholungsregel zu einem Objekt
    """
    # Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
    additional_data: dict[str, Any] = field(default_factory=dict)

    # The excludeId property
    ...
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> TnsRecurrenceAssignment:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: TnsRecurrenceAssignment
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return TnsRecurrenceAssignment()
