...
class RulesRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/tanss.app/v1/tanssEvents/rules
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/tanss.app/v1/tanssEvents/rules", path_parameters)
    
    def by_rule_id(self,rule_id: int) -> WithRuleItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.tanssApp.v1.tanssEvents.rules.item collection
        param rule_id: Id der Regel
    ...
    async def post(self,body: RulesPostRequestBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[RulesPostResponse]:
        """
        Alias von POST /api/v1/tanssEvents/rules - legt eine TANSS-Event-Regel an. Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Rechte: interner Benutzertyp; bet.ADMINISTRATE_TANSS_EVENT_RULES. Hinweise: Status CREATED. Auch unter /api/tanss.x/v1.
    ...
    async def put(self,body: TnsTanssEventRuleConfiguration, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[RulesPutResponse]:
        """
        Alias von PUT /api/v1/tanssEvents/rules - listet Event-Regeln nach Konfiguration (linkType, linkIds[], employeeIds[], triggerTypes[]). Nicht in der offiziellen Schnittstellenbeschreibung gefuehrt; vom Server so umgesetzt, gegen 10.10 geprueft. Token: module, Rollen TANSS_APP. Rechte: bet.ADMINISTRATE_TANSS_EVENT_RULES. Hinweise: Auch unter /api/tanss.x/v1.
