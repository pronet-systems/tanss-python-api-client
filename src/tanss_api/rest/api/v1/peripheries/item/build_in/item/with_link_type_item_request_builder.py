...
class WithLinkTypeItemRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/peripheries/{peripheryId}/buildIn/{linkTypeId}", path_parameters)
    
    def by_link_id(self,link_id: int) -> WithLinkItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.v1.peripheries.item.buildIn.item.item collection
        param link_id: link id of the "target" assignment (i.e. id of the pc)
