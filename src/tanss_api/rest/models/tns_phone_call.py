from __future__ import annotations
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import AdditionalDataHolder, Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

if TYPE_CHECKING:
    from .tns_phone_call_direction import TnsPhoneCallDirection
    from .tns_phone_number_identify_infos import TnsPhoneNumberIdentifyInfos
    from .tns_phone_participant import TnsPhoneParticipant

@dataclass
class TnsPhoneCall(AdditionalDataHolder, Parsable):
    """
    This object represents a phone call
    """
    # Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
    additional_data: dict[str, Any] = field(default_factory=dict)

    # (Optional) Describes the external id of the phone call, which is used by the remote phone system
    call_id: Optional[str] = None
    # True if a connection was established, otherwise will count as a "try"
    connection_established: Optional[bool] = None
    # Unix timestamp representing the begin of the phone call
    date: Optional[int] = None
    # Defines the "direction" of a phone call
    direction: Optional[TnsPhoneCallDirection] = None
    # Duration of the call (only time of actual connection) in seconds
    duration_call: Optional[int] = None
    # Total duration of the call (plus ring time) in seconds
    duration_total: Optional[int] = None
    # This object contains methods of how a phone number could be assigned to a company or employee
    from_phone_nr_infos: Optional[TnsPhoneNumberIdentifyInfos] = None
    # Phone number of the caller (= "from" number)
    from_phone_number: Optional[str] = None
    # (Optional) A group can be assigned to the call (as a string). This is an optional feature.
    group: Optional[str] = None
    # id of the phone call (in the TANSS database)
    id: Optional[int] = None
    # The phoneParticipants property
    phone_participants: Optional[list[TnsPhoneParticipant]] = None
    # (Optional) If more than one remote phone systems is used, represents the "number" of the phone system
    telephone_system_id: Optional[int] = None
    # This object contains methods of how a phone number could be assigned to a company or employee
    to_phone_nr_infos: Optional[TnsPhoneNumberIdentifyInfos] = None
    # Phone number of the called (= "to" number)
    to_phone_number: Optional[str] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> TnsPhoneCall:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: TnsPhoneCall
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return TnsPhoneCall()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        from .tns_phone_call_direction import TnsPhoneCallDirection
        from .tns_phone_number_identify_infos import TnsPhoneNumberIdentifyInfos
        from .tns_phone_participant import TnsPhoneParticipant

        from .tns_phone_call_direction import TnsPhoneCallDirection
        from .tns_phone_number_identify_infos import TnsPhoneNumberIdentifyInfos
        from .tns_phone_participant import TnsPhoneParticipant

        fields: dict[str, Callable[[Any], None]] = {
            "callId": lambda n : setattr(self, 'call_id', n.get_str_value()),
            "connectionEstablished": lambda n : setattr(self, 'connection_established', n.get_bool_value()),
            "date": lambda n : setattr(self, 'date', n.get_int_value()),
            "direction": lambda n : setattr(self, 'direction', n.get_enum_value(TnsPhoneCallDirection)),
            "durationCall": lambda n : setattr(self, 'duration_call', n.get_int_value()),
            "durationTotal": lambda n : setattr(self, 'duration_total', n.get_int_value()),
            "fromPhoneNrInfos": lambda n : setattr(self, 'from_phone_nr_infos', n.get_object_value(TnsPhoneNumberIdentifyInfos)),
            "fromPhoneNumber": lambda n : setattr(self, 'from_phone_number', n.get_str_value()),
            "group": lambda n : setattr(self, 'group', n.get_str_value()),
            "id": lambda n : setattr(self, 'id', n.get_int_value()),
            "phoneParticipants": lambda n : setattr(self, 'phone_participants', n.get_collection_of_object_values(TnsPhoneParticipant)),
            "telephoneSystemId": lambda n : setattr(self, 'telephone_system_id', n.get_int_value()),
            "toPhoneNrInfos": lambda n : setattr(self, 'to_phone_nr_infos', n.get_object_value(TnsPhoneNumberIdentifyInfos)),
            "toPhoneNumber": lambda n : setattr(self, 'to_phone_number', n.get_str_value()),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_str_value("callId", self.call_id)
        writer.write_bool_value("connectionEstablished", self.connection_established)
        writer.write_int_value("date", self.date)
        writer.write_enum_value("direction", self.direction)
        writer.write_int_value("durationCall", self.duration_call)
        writer.write_int_value("durationTotal", self.duration_total)
        writer.write_object_value("fromPhoneNrInfos", self.from_phone_nr_infos)
        writer.write_str_value("fromPhoneNumber", self.from_phone_number)
        writer.write_str_value("group", self.group)
        writer.write_collection_of_object_values("phoneParticipants", self.phone_participants)
        writer.write_int_value("telephoneSystemId", self.telephone_system_id)
        writer.write_object_value("toPhoneNrInfos", self.to_phone_nr_infos)
        writer.write_str_value("toPhoneNumber", self.to_phone_number)
        writer.write_additional_data_value(self.additional_data)
    

