from __future__ import annotations
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import AdditionalDataHolder, Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

if TYPE_CHECKING:
    from .tns_link_type import TnsLinkType
    from .tns_tanss_event_ticket_content import TnsTanssEventTicketContent
    from .tns_tanss_event_trigger_type import TnsTanssEventTriggerType

@dataclass
class TnsTanssEvent(AdditionalDataHolder, Parsable):
    """
    Describes a tanss event / activity feed item
    """
    # Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
    additional_data: dict[str, Any] = field(default_factory=dict)

    # ticket content of a tanss event
    content: Optional[TnsTanssEventTicketContent] = None
    # id of the employee who this event is for
    employee_id: Optional[int] = None
    # The id property
    id: Optional[int] = None
    # id of the assignment, which this event refers to
    link_id: Optional[int] = None
    # Enum representing the "link type" of an assignment. The id is given in the field "linkId"
    link_type: Optional[TnsLinkType] = None
    # describes the "trigger type" of an event
    trigger_type: Optional[TnsTanssEventTriggerType] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> TnsTanssEvent:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: TnsTanssEvent
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return TnsTanssEvent()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        from .tns_link_type import TnsLinkType
        from .tns_tanss_event_ticket_content import TnsTanssEventTicketContent
        from .tns_tanss_event_trigger_type import TnsTanssEventTriggerType

        from .tns_link_type import TnsLinkType
        from .tns_tanss_event_ticket_content import TnsTanssEventTicketContent
        from .tns_tanss_event_trigger_type import TnsTanssEventTriggerType

        fields: dict[str, Callable[[Any], None]] = {
            "content": lambda n : setattr(self, 'content', n.get_object_value(TnsTanssEventTicketContent)),
            "employeeId": lambda n : setattr(self, 'employee_id', n.get_int_value()),
            "id": lambda n : setattr(self, 'id', n.get_int_value()),
            "linkId": lambda n : setattr(self, 'link_id', n.get_int_value()),
            "linkType": lambda n : setattr(self, 'link_type', n.get_enum_value(TnsLinkType)),
            "triggerType": lambda n : setattr(self, 'trigger_type', n.get_enum_value(TnsTanssEventTriggerType)),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_object_value("content", self.content)
        writer.write_int_value("employeeId", self.employee_id)
        writer.write_int_value("id", self.id)
        writer.write_int_value("linkId", self.link_id)
        writer.write_enum_value("linkType", self.link_type)
        writer.write_enum_value("triggerType", self.trigger_type)
        writer.write_additional_data_value(self.additional_data)
    

