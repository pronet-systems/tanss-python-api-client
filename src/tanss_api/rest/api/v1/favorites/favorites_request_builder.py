...
class FavoritesRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/favorites", path_parameters)
    
    def by_link_type_id(self,link_type_id: int) -> WithLinkTypeItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.v1.favorites.item collection
        param link_type_id: Link type to filter favorites by (e.g. company, ticket, employee).
