...
class WithLinkTypeItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/v1/checklists/{checklist-id}/convert/adhoc/{linkTypeId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/checklists/{checklist%2Did}/convert/adhoc/{linkTypeId}", path_parameters)
    
    def by_link_id(self,link_id: int) -> WithLinkItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.v1.checklists.item.convert.adhoc.item.item collection
        param link_id: id of the link target (e.g. the ticket).
