...
class ToRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/v1/permissions/permissionPackage/{permissionPackageId}/to
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/permissions/permissionPackage/{permissionPackageId}/to", path_parameters)
    
    def by_employee_id(self,employee_id: int) -> WithEmployeeItemRequestBuilder:
        """
        Gets an item from the tanss_api.rest.api.v1.permissions.permissionPackage.item.to.item collection
        param employee_id: Id of the employee the package permissions should be removed from.
