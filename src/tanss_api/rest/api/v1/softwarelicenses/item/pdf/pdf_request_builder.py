...
class PdfRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/v1/softwarelicenses/{id}/pdf
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/softwarelicenses/{id}/pdf{?internal*}", path_parameters)
    ...
    async def get(self,request_configuration: Optional[RequestConfiguration[PdfRequestBuilderGetQueryParameters]] = None) -> Optional[PdfGetResponse]:
        """
        Generates a printable detail sheet for one license — fields, expiry, vendor, key, assignments. The `internal`flag toggles between the customer-facing layout and the internal version (which adds remarks/notes that are notmeant to leave the company). Honours the `Accept` header to either return the PDF inline or as the standard JSONenvelope with a download URL.
