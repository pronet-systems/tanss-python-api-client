...
class WithTypeItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/v1/remoteSupports/configuration/{type}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/remoteSupports/configuration/{type}", path_parameters)
    
    async def get(self,request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[WithTypeGetResponse]:
        """
        Returns the stored configuration parameters (endpoint, credentials placeholders, fetching flags) for the requested provider`type` — e.g. `TEAMVIEWER`, `ANYDESK`, `ISL_ONLINE`. Intended for the OSK admin screen, so it requires either the`ONLINE_SYSTEM_CONFIGURATION` permission or technician/freelancer role.
