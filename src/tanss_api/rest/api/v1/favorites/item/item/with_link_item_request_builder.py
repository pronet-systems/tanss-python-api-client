...
class WithLinkItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/v1/favorites/{linkTypeId}/{linkId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/v1/favorites/{linkTypeId}/{linkId}", path_parameters)
    
    async def delete(self,request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> None:
        """
        Removes the favorite of the given link type (`linkTypeId`, e.g. company,ticket, employee) pointing at `linkId` from the currently logged-in user'sfavorites list.
    ...
    async def post(self,request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[WithLinkPostResponse]:
        """
        Adds an entity (identified by `linkTypeId` + `linkId`, e.g. a ticket orcompany) to the currently logged-in user's favorites list.
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
